 ...
 * 令牌对账。
 *
 * 色值在三处出现：docs/design/design-tokens.md（源）、src/styles/_vars.scss（样式用）、
...
/* ============ 1. _vars.scss ↔ colors.js ============ */

L('=== 1. _vars.scss 与 colors.js 对账 ===')
const scss = readFileSync(join(root, 'src/styles/_vars.scss'), 'utf8')
const scssVars = {}
...
chk(uncovered.length === 0, `_vars.scss 里没有漏登记的色${uncovered.length ? `：${uncovered.join(', ')}` : ''}`)

/* ============ 2. design-tokens.md ↔ _vars.scss ============ */

L('\n=== 2. design-tokens.md 与 _vars.scss 对账 ===')
...
for (const [name, hex] of Object.entries(docVars)) {
  if (!(name in scssVars)) {
    L(`    ✗ 文档有 --${name} 但 _vars.scss 里没有 $${name}`)
    docMismatch++
  } else if (scssVars[name] !== hex) {
    L(`    ✗ --${name}：文档 ${hex} vs _vars.scss ${scssVars[name]}`)
    docMismatch++
  }
}
chk(docMismatch === 0, `文档里的 ${Object.keys(docVars).length} 个色值与 _vars.scss 一致`)
...
// colors.js 和 icons.js 本来就是放色值的地方，_vars.scss/tokens.scss 是源
...
// 两件事一起查：
//
// ① **不许出现新字号**。收口前有 90 多处硬编码，一像素的区分纯粹是噪音，
//    还有十几处 11px，低于 design-tokens 自己定的 12px 下限。
//    梯子只有八级（typography.js 的 FS_BASE），要第九级先去改那份文档。
//
// ② **不许用 $fs-* SCSS 变量**。SCSS 变量在编译期就写死成一个值了，
...
//
//    ⚠️ 取的是**第三列（px）**，不是第二列（rpx）。web 端单位换成了 px
//    （rpx ÷ 2，2026-08-30 用户定），而 rpx 那一列文档里留着 —— 那是小程序时代的值。
//    表格行形如： | `--fs-hero` | `46` | 23px | 700 | ... |
const fsDoc = {}
for (const m of doc.matchAll(/\|\s*`--fs-([a-z]+)`\s*\|\s*`\d+`\s*\|\s*(\d+)px/g)) {
  fsDoc[m[1]] = Number(m[2])
}
const fsDiff = []
...
chk(scssFs === 0, '_vars.scss 里没有 $fs-* 变量（字号只有 typography.js 一个源）')
