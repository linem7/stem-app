...
import { DEV_OPENID } from '../utils/env.js'
import { readLocal, writeLocal } from '../utils/storage.js'

/**
 * 上一次那个 token 是发给哪个开发账号的。
 *
 * 🔴 没有它会踩一次：`npm run dev:account` 换了 `VITE_DEV_OPENID` 之后，
 * 浏览器里还存着**上一个账号**的 token，`bootstrap` 有 token 就直接拉档案，
 * 拉回来的是那个**没激活**的老师 —— 表现是侧边栏一直写着
 * 「这个小程序目前只开放给合作园的老师」，而首页看起来完全正常。
 *
 * 临时的，跟 DEV_FAKE_LOGIN 一起，真登录页搬过来就删。
 */
const DEV_MARK = 'stem_dev_token_for'
...
export async function bootstrap() {
  ...
  // 换了开发账号就把旧 token 扔掉，重新登一次（见 DEV_MARK 的注释）
  if (DEV_OPENID && readLocal(DEV_MARK) !== DEV_OPENID) clearToken()
  try {
    if (getToken()) {
      // 有 token 先试着直接拉档案，省一次登录请求
      session.teacher = await getMe()
    } else {
      const data = await apiLogin()
      session.teacher = data.teacher
      if (DEV_OPENID) writeLocal(DEV_MARK, DEV_OPENID)
    }
  } catch (err) {
