<template>
  <s-page tab="me">
    <!--
      顶栏（带返回箭头）。原来这一页**没有 #top 插槽，所以没有返回键** ——
      全局是 navigationStyle: custom，微信自己那个返回箭头不存在，
      s-topbar 是唯一的返回入口。她从「我的」点进来，只能靠底部 tab 绕回去。

      **不给它标题** —— 底下那个大标题就是「可以换额度的事」，
      顶栏再写一遍是同一句话占两行。别的页那样写是因为两处文字不同
      （revise 顶上「改一改」、底下「哪里不对？说给我听」）。
    -->
    <template #top>
      <s-topbar />
    </template>
