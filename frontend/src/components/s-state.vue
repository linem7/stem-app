<template>
  <view class="st" :class="`st--${kind}`">
    <text class="st__t">{{ shownText }}</text>
    ...
    <!--
      ...
      **只说否定的那一面，网通着就一个字都不说** —— 第一版写的是
      ...
      「wifi 好得很，是后端连不上」。真断网时也照样显示（实测出来的，
      uni 的 H5 实现不烧 window 的 offline 事件）。
      这一屏正是她最需要准确信息的时候，宁可少说一句。
    -->
    <text v-if="!net.online" class="st__live">还是没有网络</text>

    <view class="st__ops">
      <s-button v-if="actionLabel" :label="actionLabel" :variant="kind === 'empty' ? 'primary' : 'plain'" :arrow="kind === 'empty'" @press="$emit('action')" />
 ...
 * 话术统一在后端，改措辞不用重发小程序（api-spec 第 0 节）。
 * 这个组件自己只写一句话 —— 上面那行「网回来了 / 还是没有网络」，
 * 因为请求根本没到后端，后端也就没机会说话。
 */
...
/* 珊瑚色只用 -deep 档 —— 亮色不做文字（design-tokens 规则 1） */
.st__live {
  display: block;
