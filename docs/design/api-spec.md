...
|---|---|---|
| 身份 | 从四级下拉里认领一个位置 | 自己填 |
| 必填 | 码 + `roster_entry_id` + 手机号 + 密码 | 码 + **姓名 + 园所名 + 省 + 市 + 园所类型** + 手机号 + 密码 |
| 落库 | 名单行转 `claimed`、`claimed_by` 指向她 | **新插一行 `teacher_roster`**（见下） |
| 地区 | 从园所带出来 | 她自己选（**下拉，不是自由填**） |
...
| 字段 | 取值 |
|---|---|
| `birth_year` | 4 位年份。**存年份不存年龄** —— 年龄会过期，三年后那份数据就变成「当时 35 岁」 |
