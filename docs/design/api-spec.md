...
```json
// 请求
{ "phone": "13812345678", "password": "她自己设的那个" }
...
{ "ok": true, "data": {
  ...
  "expires_in": 2592000,
  "teacher": {
    "id": 1, "nickname": null, "avatar_url": null,
    "kindergarten_name": null, "age_group": "中班", "teaching_years": null,
    "class_name": "中一班", "position": "主班",
    "profile_completed": false,
    "activated": true, "agreed": false
  }
}}
```

**手机号在这个系统里只是用户名** —— 不用于联系、不进 AI 提示词、不下发到页面、不进日志。
响应里**永远没有 `phone` 和 `real_name`，包括老师自己的**。

输入宽容：空格、横线、括号、`+86` 前缀、全角数字都认（`utils/phone.js`）。
认不出来回 `手机号看起来不对，是 11 位数字`。

🔴 **手机号不存在和密码错必须回同一句话** —— `手机号或密码不对`。
分开说等于送出一个「查这个号注册过没有」的接口，而老师最怕的正是
「园长知道我用了 AI 写教案」。两条路都会先跑一次哈希比较，让耗时也一致。

🔴 **`auth: false`**（前端）。这一次请求本身还没有 token，
...
前端据此决定落在哪个页：**`agreed=false` → 协议页**；为 true 才进主流程。
（`activated` 现在恒为 true，见下面 `POST /auth/activate` 的说明。）

**响应里永远没有 `real_name`，包括老师自己的** —— 姓名只活在服务端与管理后台。
**手机号同样不下发**，它只是登录用的用户名。

### `POST /auth/activate` · 建账号（2026-09-20 新增）
...
{ "code": "stem 4k7p qx3m",
  "roster_entry_id": 42,
  "phone": "13812345678",
  "phone_confirm": "13812345678",
  "password": "至少 6 位" }

// 响应
{ "ok": true, "data": {
  "kind": "activate",
  "token": "jwt…",              // 建完直接给，免得她刚设完密码又要立刻用它登一次
  "expires_in": 2592000,
  "teacher": { … },
  "quota": { "text": {…}, "image": {…} },
...
| | 证明什么 | 从哪来 |
|---|---|---|
| **兑换码** | 你有资格进来 | 问卷星在她提交答卷后当场发 |
| **从名单里选** | 你是**哪一个** | 省 → 市 → 园所 → 班级·岗位·姓氏，四级选下来 |
| **手机号 + 密码** | 你下次怎么进得来 | 激活那一屏她自己设 |

🔴 **码不限定园所、也不限定「这批人」**（2026-09-21 用户定）。
码的职责是**「证明你有资格进来」**，不是「证明你是这个园的」。
所以任意有效未使用的码都能查到任何园的位置，也能走「不在名单之内」那条路。
**门还在** —— 没码的人什么都做不了。扩的是门后面的白名单，不是取消门。

### 两条激活路径（2026-09-21 新增第二条）

| | 白名单里的老师 | 不在名单的人 |
|---|---|---|
| 身份 | 从四级下拉里认领一个位置 | 自己填 |
| 必填 | 码 + `roster_entry_id` + 手机号 + 密码 | 码 + **姓氏 + 省 + 市 + 园所类型** + 手机号 + 密码 |
| 落库 | 名单行转 `claimed`、`claimed_by` 指向她 | **新插一行 `teacher_roster`**（见下） |
| 地区 | 从园所带出来（只有实际有园的省市） | 她自己选（**完整中国省市**，见下） |

🔴 **自填这条路只要姓氏、不要全名，也不填园所名**（用户 2026-09-21 定）：

| 收什么 | 为什么 |
|---|---|
| **只要姓氏** | 之后系统里就叫她「{姓氏}老师」。跟白名单那条路同一个立场 —— 姓名只给姓氏，认出自己只需要一个字，收集全名是没有必要的暴露 |
| **不填所在幼儿园名称** | 收集它没有用途。少收一样可识别信息就少一整套合规义务 |

🔴 **地区用完整的中国省市，不是「实际有园的省市」**（用户 2026-09-21 定：
「地区信息应该遵循中国的省市写法，而不仅仅是当前已有的地区」）。

- 数据在 `backend/src/utils/chinaRegions.js` 和 `frontend/src/utils/chinaRegions.js`
  （**两份副本，改的时候一起改**），31 个省级 + 341 个地级市，
  从公开行政区划数据生成而不是手打的
- 后端拿它做校验（`isKnownRegion`）。**不能拿「库里已有园所的省市」校验** ——
  她不在名单里，可能来自任何地方，那样她明明填对了也会被拒
- 白名单那条路**仍然是「只有实际有园的省市」**（`listOpenRegions` 从园所表 DISTINCT）
  —— 那是对的：她要在名单里找自己，而名单只覆盖有园的地方

⚠️ **「不在名单」的人也要插一行 `teacher_roster`**，不是直接建 `teachers`。
理由是 `teacher_ref` 是**「人」这一层**的身份，研究上要按它归组，
自填的人也得有一个。那一行：

```
teacher_ref       新的（自己一个人一个 ref）
kindergarten_id   NULL      ← 不属于任何园所
real_name         **她填的姓氏**（一个字）。⚠️ 白名单那条路上这一列存的是全名，
                  而 `surnameOf()` 取第一个字 —— 两条路都取第一个字，结果一致
...
自填的人没法回答「发了几个码、兑了几个」，因为码不再绑定批次。

🔴 **手机号要传两遍**（`phone` 和 `phone_confirm`），**后端自己再比一次**。
11 位打错一位是常事，而后果特别隐蔽：她下次登录输的是**正确的号**、
库里存的是**打错的号**，进不去，且她完全不知道哪里出了问题。
只在前端比不够 —— 绕过前端就会存进一个错的号。

**为什么还要从名单里选**：园所 / 班级 / 岗位是研究的自变量，
也是后台「发了几码、兑了几个」的跟进依据。手机号当一个用户名就够了，
它不承担「证明你是哪个班的主班」这件事。

**要认下来的代价**：从列表里选是一个**表单字段，不是钥匙**，谁有码都能滚到任何一行。
可以接受，因为同事之间冒领没有收益（她自己填问卷也能拿到同样额度），
而真会发生的「手滑选错同班另一位」是**可查可改**的 —— 名单那一行的 `claimed_by` 记着是谁。

流程：码 `unused` → 名单行 `pending` → 手机号没注册过 → **全对**才在一个事务里
建 `teachers` 行、标记码已用、名单 `claimed`、发首笔额度。`activated_at` 当场置上。

🔴 **校验失败绝不能消耗那个码。** 顺序是**先全部校验，再全部落库**。
她进不来时只会说一句「用不了」，没有别的线索 —— 这一条有回归守着。
...
| 手机号注册过 | 这个手机号已经注册过了。要是你之前激活过，用手机号登录就行 |
| 两次手机号不一样 | 两次填的手机号不一样，再核对一下 |
| 自填的地区不在下拉里 | 地区从下拉里选一下 |
| 自填的园所类型不在白名单 | 园所类型从选项里选一下 |

### `POST /auth/activate` 的请求体（2026-09-21 扩充）

```jsonc
// 路径一：白名单里的老师
{ "code": "stem 4k7p qx3m", "roster_entry_id": 42,
  "phone": "13800000000", "phone_confirm": "13800000000",
  "password": "……" }

// 路径二：不在名单的人
{ "code": "stem 4k7p qx3m",
  "real_name": "张三", "kindergarten_name": "朝阳区第二幼儿园",
  "province": "北京", "city": "北京", "ownership": "公办",
  "phone": "13800000000", "phone_confirm": "13800000000",
  "password": "……" }
```

**判据是 `roster_entry_id` 有没有传** —— 传了走路径一，没传走路径二。
⚠️ **两条路都必须带 `phone_confirm`**，后端自己再比一次（对所有人一视同仁）。
⚠️ `province` / `city` **必须在 `kindergartens` 里出现过的值**里选，
`ownership` 必须在白名单里 —— **不许自由填**，否则地区数据会脏成
「北京 / 北京市 / 北京朝阳」，研究上没法按地区分组。

⚠️ **`activated` 现在是恒真的。** `teachers` 行只有这一个接口会建，
而它当场就置 `activated_at` —— 所以 `requireActivated` 的**前半段**（`!activated_at`）
在 web 端已经走不到了。
**后半段（`!agreed_at`）还活着，而且是必须的**：协议那道闸门在服务端就靠它。
没删前半段，因为删它要动 `server.js` 的挂载顺序；记在这里等一起收。

### `POST /auth/redeem` · 续兑（要登录）

```json
{ "code": "stem 4k7p qx3m" }
// → { "ok": true, "data": { "kind": "topup", "teacher": {…},
//      "quota": {…}, "granted": { "text": 5, "image": 3 } } }
```

**只要码，身份一个字段都不动。** 她已经登录了，不需要再选一次自己是谁。
入口在「我的」里。

⚠️ 它跟 `/auth/activate` 是**两个接口**，因为调用它们的人不同：
续兑的人带着 token，激活的人还没有账号。老师端看到的都是一个输入框，
**分岔在代码里，不在她的操作里**。

⚠️ **换绑（`kind=rebind`）整条路已经作废**：它存在的前提是「身份就是 openid，
换微信 = 一个全新账号」。web 端换设备只要手机号 + 密码，没有「换绑」这回事。
代码和 `account_rebinds` 表还在（后台也还能建换绑码），**没删，等一起收**。
...
| `teachers.id` | **账号** | 换设备用手机号 + 密码进来，脱掉的只是 token |
...
激活那一屏的**四级**选择器（2026-09-21 从三级改成四级：
地区 → 园所 → 班/岗位 → 人）。**必须带一个有效的码才给数据** ——
不设这道门，任何人打开网页就能看到一整个园的老师名单。

```jsonc
// 第一级：有哪些地区（只列还有未认领位置的园所在的地区）
{ "code": "stem 4k7p qx3m" }
// → { "regions": [ { "province": "北京", "cities": ["北京"] } ] }

// 第二级：这个省有哪些市
{ "code": "stem 4k7p qx3m", "province": "北京" }
// → { "cities": ["北京"] }

// 第三级：这个市有哪些园（只列还有未认领位置的）
{ "code": "stem 4k7p qx3m", "province": "北京", "city": "北京" }
...
**四个参数是「给到哪一级就回哪一级」**，不是四选一 ——
第一级不带参数、第二级只带 `province`、第三级带 `province` + `city`、
第四级带 `kindergarten_id`。判据是**最后一个有值的参数**。

- 只回 **`pending`** 的位置。已经被认领的不出现 —— 她看到一个选不了的选项只会困惑
- **姓名只给姓氏**。她认自己够了，把同事全名摊给任何拿到码的人是没必要的暴露
- 码无效 / 已用 / 作废 → 直接报错，**不回任何名单数据**
- 🔴 **这个接口是公开的，不要求登录**（2026-09-20 改）。
  原来它挂在 `requireAuth` 后面，那在小程序时代没问题 —— 人人一进来就静默登录、都有 token。
  web 端没有静默登录，「拉名单要有 token」和「有 token 要先有账号」合起来就是死循环。

⚠️ **`kindergarten_id` 那个分支的行为不许改**。`redeem.vue` 的 `pickKg` 依赖它
（传园所 id 就回 entries）。四级是**加参数**，不是换语义。

一个班有两个配班时，`surname` 就是区分它们的那一项。
两个配班同姓的话再加 `note`（后台录名单时可以填「配班（靠窗）」这种）。

🔴 **码不限定园所。** 任意有效未使用的码都能查到任何园的位置 ——
这是 2026-09-21 用户定的：码的职责是「你有资格进来」，不是「你是这个园的」。
**但没有码就什么都查不到**，门还在。
...
| 删 | 留 |
|---|---|
| 对话、教案、版本、配图（连磁盘文件）、记忆 | 那一行的 id（用来挂住右边这两样） |
| **手机号、密码哈希**、姓名、昵称、头像、园所班级岗位 | 额度台账、已提交的建议与评价 |

留下的那些记录从此不再关联到任何姓名和手机号。

⚠️ **「拒绝再次登录」这道闸现在是窄的**（2026-09-20 记）。
手机号被真删了，所以 `WHERE phone = $1` 找不到注销过的行 ——
她拿一个新码、用同一个号**可以重新报名**。
要堵死它就得留一份手机号哈希，那跟「删掉全部数据」直接矛盾。
...
### `POST /me/profile` · 完善信息领额度（2026-09-21 新增）

老师主动填**五项**档案，换一笔额度。**这是第一条「她主动提供信息换东西」的接口** ——
跟记忆抽取是两回事：**这里是她自己填的，不是 AI 从她聊天里推断的**。
...
**发 10 教案 / 5 配图**，写一条 `quota_grants`（`reason = '完善信息'`）。

🔴 **为什么是「出生年月」而不是「出生年」、也不是「生日」**（2026-09-21 用户定）：

- **不存年龄** —— 年龄会过期，三年后那份数据就变成「当时 35 岁」，研究要的是「她多大」
- **不存日** —— 日是不收的信息。存成 `DATE` 的 `1995-12-01` 会让
  「她生日是 12 月 1 日」这个**不存在的信息**看起来像是收上来了。
  **不收的东西不该以任何形式进库**，哪怕是补一个默认的 01
- 所以库里是 `VARCHAR(7)` 存 `'1995-12'`，有一条 CHECK 守着格式

界面上是一个 `type="text"` 的输入框、提示语「如 1995-12」。
**不用 `type="month"`** —— 它在 iOS 上弹的是只有滚轮的日期选择器，
而这里要的是能直接打字的年月。

⚠️ 这五项**也进了「我的 → 个人档案」**（用户 2026-09-21 要求），
那条路走的是 `PATCH /me`。所以 `PATCH /me` 的白名单里**也加了 `birth_month`**，
并且**同样守了一次格式** —— 只守 profile 这条的话，她改档案时能存进
`1995-13`，而数据库那条 CHECK 会直接 500。

🔴 **只能领一次。** 判据**不是**「四个字段填全了」——
那样她把学历从「大专」改成本科会**再领一次**。
判据是查 `quota_grants` 里有没有 `reason = '完善信息'` 的记录：

```sql
SELECT 1 FROM quota_grants WHERE teacher_id = $1 AND reason = '完善信息'
```

⚠️ 注意这跟「改档案」是**两个接口**（`PATCH /me`）。`PATCH /me` 改档案不发额度。
**两条路分开**，否则她反复改档案就能反复领。

⚠️ `quota_grants.reason` 是 `VARCHAR(64)`。「完善信息」4 个字符，安全。
⚠️ `getQuota` 是对 `quota_grants` 求 `SUM(delta_text)` —— 所以新发的**立刻生效**，
不用改 `quota.js`。
