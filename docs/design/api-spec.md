...
```json
{ "code": "stem 4k7p qx3m", "phone": "13800001234" }   // phone 只有首次激活才要
...
只用它当凭据等于没有门槛 —— 任何知道 A 老师号的人都能领走 A 的名额，
而 A 之后自己输会看到「已被认领」，被顶掉了还查不出是谁顶的。

⚠️ **码不绑在名单那一行上，两把钥匙必须相互独立。** 一旦把码绑到某个手机号，
问卷星发的随机码就对不上她的号，「答卷后自动发码」这条路当场就断。**别改回去。**

流程：码存在且 `unused` → 手机号在名单里且 `pending` → **两个都对**才在一个事务里
...
| 码对、号不在名单 | 名单里没有这个手机号，确认一下有没有打错，或者问园长你在不在名单里 |
| 号已被认领 | 这个手机号已经激活过一个账号了（后台查得到是哪个 openid 认领的） |

#### 续兑（任务奖励）

**只要码，不问手机号** —— 她已经被识别过了。**身份一个字段都不动**：
码上的手机号姓名一律忽略、连查重都不做。再拿码上的信息去覆盖只会把她的资料改坏。
...
激活的第二把钥匙（见第 1.5 节）。合作园给一份名单，我录进来；
老师激活时输的手机号跟这里核对。
...
{ "items": [ {
  "id": 7,
  "phone": "138****1234",     // **一般管理员打码**，跟老师详情同一条纪律
  "phone_masked": true,
  "real_name": "王老师", "kindergarten": "阳光幼儿园",
...
```jsonc
{ "text": "王老师, 13800001234, 小一班, 主班, 小班\n李老师, 13800005678, 中二班, 配班, 中班",
  "kindergarten_id": 1,
  "dry_run": true }
// → { "rows": [ { "line": 1, "phone": "138...", "real_name": "王老师",
//                 "ok": true, "reason": null } ],
...
- 手机号清洗掉空格和横线，必须是 11 位且以 1 开头；不合格 → `invalid` + 报行号
- 名单里已有这个号 → `duplicate`，**跳过而不是覆盖**（覆盖会悄悄改掉一个人的身份）
- 只有姓名和手机号是必需的，班级/岗位/年龄班可缺
