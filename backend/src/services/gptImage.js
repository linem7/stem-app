/**
 * gpt-image 系列图片生成（经 12ai 中转，OpenAI images 接口的形状）。
 *
 ...
 * 2. **尺寸不限于文档列的那几档**。文档早期给的是 1248x1248 / 1792x1008 / 1008x1792，
 *    实测 1536x2048 照收，返回的就是 1536x2048 —— 所以打印用的尺寸（长边 2048、
 *    约 A4 250 DPI）能原样保留。照抄那几档等于把打印分辨率砍掉一半
 ...
 *
 * ─────────────────────────────────────────────────────────────
 * 2026-09-21：12ai 那边多了 `gpt-image-2.5-flare` 和 `gpt-image-2.5-sunburst`。
 * 用户选了 **flare**（文档说它「兼顾画质与速度」，且比 gpt-image-2 延迟约减半），
 * **质量档保持 `high` 不变** —— 只换模型、不换质量，这样质量变化能确定是
 * 模型的功劳，快慢也好跟现在对比。
 *
 * ⚠️ **flare 支持更多档位但这里没用到**：它多了 `xhigh` / `max`
 * （sunburst 和 flare 都是 low/medium/high/xhigh/max/auto）。
 * 想试更高档**不用改代码** —— 后台模型管理里那一行的「质量」留空时走
 * `auto`，想固定就在 `options` 里给 `quality`。改 `.env` 的 `IMG_QUALITY`
 * 只对**新机器首次播种**生效（播种之后就不再读 .env 了）。
 *
 * ⚠️ **`size` 的三条约束核过了**（2026-09-21）：两边必须是 16 的倍数、
 * 长短边比 ≤ 3:1、总像素 655,360–8,294,400。`imagePurpose.js` 里那九个
 * 尺寸全部合法，所以换 flare 不会因为尺寸被 400。**加新尺寸时要重新核**。
 * ─────────────────────────────────────────────────────────────
 */
