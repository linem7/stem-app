...
/**
 * 最高学历与职称的白名单（018 迁移加的两列）。
 *
 * 放在这里而不是 me.js：这两个清单**前后端都要用**（后端校验、前端摆胶囊），
 * 而 POSITIONS 已经在这个文件里了 —— 同一类东西放同一处，
 * 改一档不用去两个文件找。前端那份是副本，`test:api` 不查它们，
 * **所以改这里记得同步 `frontend/src/pages/me/me.vue`**。
 *
 * ⚠️ 职称序列 2022 年后并进中小学序列，各省落地节奏不一样。
 * 用户是幼教研究者，这两个清单该怎么分档他比我准 —— 改就改这两个数组。
 */
export const EDUCATIONS = ['中专及以下', '大专', '本科', '硕士及以上'];

/**
 * 职称。**五档**（2026-09-21 用户定，从六档合并）。
 *
 * 这是中小学教师职称序列的通行粗分：
 *   初级（原三级 + 二级）→ 中级（原一级）→ 副高级（原「高级教师」）→ 正高级
 *
 * 🔴 **「高级教师」写成「副高级」不是措辞问题。** 中小学的「高级教师」
 * 在序列上对应大学的副教授，写成「高级」会让人以为它比正高还高一档，
 * 而它下面还有正高级。用户 2026-09-21 特意补了「应该要有副高级」这一句。
 *
 * ⚠️ 「未评级」是她主动选的值，跟 NULL（没填过）**不是一回事** ——
 * 别在任何地方把 NULL 显示成「未评级」。
 */
export const TITLES = ['未评级', '初级', '中级', '副高级', '正高级'];
